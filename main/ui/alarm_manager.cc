...
void AlarmManager::OnRingTimer() {
    ...
    // mặc định: GA
    const std::string_view* ogg = &Lang::Sounds::OGG_ALARM1;
    ...
    if (id == "alarm1") {
        ...
        // giá trị lạ → ép về ALARM1
        ogg = &Lang::Sounds::OGG_ALARM1;
    }

    // Fallback: ALARM1 lỗi → IPHONE → lại về ALARM1
    if (ogg->empty()) {
        ESP_LOGW(TAG, "Selected ringtone is empty, fallback to IPHONE");
        ogg = &Lang::Sounds::OGG_IPHONE;
        if (ogg->empty()) {
            ESP_LOGW(TAG, "IPHONE is also empty, fallback back to ALARM1");
            ogg = &Lang::Sounds::OGG_ALARM1;
        }
    }
