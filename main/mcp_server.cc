...
void McpServer::AddCommonTools() {
    ...
    // ---- Wallpaper control ----
    {
        AddTool("self.wallpaper.apply",
            "Đổi hình nền theo số (bắt đầu từ 1). Hiện chỉ hỗ trợ hiệu ứng 'fade_black' (mờ dần). Các hiệu ứng khác sẽ tự động dùng fade_black.",
            PropertyList({
            ...
            [](const PropertyList& properties) -> ReturnValue {
                ...
                // Map mọi thứ về fade_black để an toàn
                (void)effect_str;

                size_t idx0 = (one_based > 0) ? (size_t)(one_based - 1) : 0;
                return WallpaperManager::GetInstance().ApplyWithEffect(idx0, fx);
            });
    }
    ...
    // ---- Wallpaper timer control ----
    {
        AddTool("self.wallpaper.set_interval",
            "Đặt thời gian tự động thay hình nền. Các giá trị hợp lệ: 30s, 1m, 3m, 5m, 15m, 30m, 1h.",
            PropertyList({
            ...
            [](const PropertyList& props) -> ReturnValue {
                ...
                else if (dur == "1h" or dur == "60m") seconds = 3600;

                WallpaperManager::GetInstance().EnableAutoRotate(true, seconds);
                ESP_LOGI(TAG, "Wallpaper auto-rotate interval set to %d seconds", seconds);
                return true;
            });
    }
    ...
    AddTool("self.alarm.set",
        ...
        PropertyList({
            ...
            Property("ringtone", kPropertyTypeString, "ga"),
        ...
        [&alarm](const PropertyList& p) -> ReturnValue {
            ...
            std::string ringtone = "ga";
            try { ringtone = p["ringtone"].value<std::string>(); } catch (...) {}
            if (ringtone != "ga" && ringtone != "alarm1" && ringtone != "iphone") ringtone = "ga";
